...
app.post("/api/sites", upload.single("files"), async (req, res, next) => {
  try {
    ...
    applyAdvancedSettings(site, { accessListId, compression: req.body.compression, hstsSubdomains: req.body.hstsSubdomains === "true", customConfig: req.body.customConfig });
...
app.patch("/api/sites/:id", async (req, res, next) => {
  try {
    ...
    applyAdvancedSettings(site, { accessListId: req.body.accessListId, compression: req.body.compression, hstsSubdomains: req.body.hstsSubdomains, requestHeaders: req.body.requestHeaders, responseHeaders: req.body.responseHeaders, customConfig: req.body.customConfig });
    await syncCaddy();
    await saveSites();
...
app.get("/api/system/status", async (req, res, next) => { try { if (req.user.role !== "administrator") return res.status(403).json({ error: "Administrator access is required." }); const backup = settings.backups || {}; const retention = settings.logsRetention || {}; const paths = [dataDir, path.join(dataDir, "database"), sitesDir, uploadDir, caddyDir, iconsDir, logsDir, backupsDir, defaultSiteDir, certificatesRoot].map(target => ({ path: target, exists: fs.existsSync(target), type: "directory" })); let storageInfo = null; try { const stat = await fsp.statfs(dataDir); storageInfo = { availableBytes: Number(stat.bavail) * Number(stat.bsize), totalBytes: Number(stat.blocks) * Number(stat.bsize) }; } catch {} res.json({ version: appVersion, uptime: process.uptime(), memory: process.memoryUsage(), database: { path: path.join(dataDir, "database", "site-gateway.sqlite"), healthy: Boolean(storage?.integrity?.()) }, paths, storage: storageInfo, jobs: [{ name: "Upstream health checks", schedule: "Every 60 seconds", enabled: true }, { name: "Scheduled backup", schedule: `${backup.enabled ? `Enabled · ${backup.frequency} at ${backup.hour}:00` : "Disabled"}`, enabled: Boolean(backup.enabled) }, { name: "Scheduled log pruning", schedule: retention.pruningEnabled ? "Enabled · every 15 minutes" : "Disabled", enabled: Boolean(retention.pruningEnabled) }, { name: "Access log import", schedule: "Every 30 seconds", enabled: true }] }); } catch (error) { next(error); } });
