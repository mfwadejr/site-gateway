...
function hostedCard(site) {
  ...
  const upstream = !site.enabled ? "Monitoring paused" : !site.upstream ? "Upstream check pending" : site.upstream.status === "healthy" ? `Upstream ${site.upstream.httpStatus} · ${site.upstream.responseMs} ms` : `Upstream unavailable · ${escapeHtml(site.upstream.error || "check failed")}`;
