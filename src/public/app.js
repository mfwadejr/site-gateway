...
function proxyCard(proxy) {
  ...
  const check = proxy.upstream; const detail = check ? `Expected ${escapeHtml(proxy.healthExpected || "200-499")} · Received ${check.httpStatus || "no HTTP response"} · ${check.responseMs ?? "—"} ms · ${check.attempts || 1} attempt${(check.attempts || 1) === 1 ? "" : "s"} · ${escapeHtml(formatTime(check.checkedAt))}` : "No health check recorded yet";
  return `<article class="site-card proxy" data-id="${proxy.id}" data-kind="proxy"><div class="card-top"><div class="site-icon">${iconMarkup(proxy)}</div>${menu}</div><h2>${escapeHtml(proxy.name)}</h2><p class="address">${escapeHtml(proxy.target)}</p><p class="gateway-address ${proxy.tls !== "http" ? "secure" : ""}">${escapeHtml(publicUrl(proxy))}</p><p class="upstream-copy ${proxy.upstream?.status === "unhealthy" ? "bad" : ""}">${upstream}</p><details class="upstream-diagnostics"><summary>View diagnostics</summary><p class="upstream-detail">${detail}</p>${check?.error ? `<p class="upstream-detail bad">${escapeHtml(check.error)}</p>` : ""}</details><p class="access-summary">${escapeHtml(access)}</p><div class="card-footer"><span class="status-pill"><span class="status-dot ${status}"></span>${status === "error" ? "Needs attention" : status[0].toUpperCase() + status.slice(1)}</span><div class="card-actions">${toggle}<a class="launch" href="${publicUrl(proxy)}" target="_blank" rel="noopener" aria-label="Open ${escapeHtml(proxy.name)}">↗</a></div></div></article>`;
}
