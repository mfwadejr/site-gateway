...
function hostedCard(site) {
  ...
  const upstream = !site.enabled || site.upstream?.status === "unmonitored" ? "Monitoring paused" : !site.upstream ? "Upstream check pending" : site.upstream.status === "healthy" ? `Upstream ${site.upstream.httpStatus} · ${site.upstream.responseMs} ms` : `Upstream unavailable · ${escapeHtml(site.upstream.error || "check failed")}`;
...
function proxyCard(proxy) {
  ...
  const upstream = !proxy.enabled || proxy.upstream?.status === "unmonitored" ? "Monitoring paused" : !proxy.upstream ? "Upstream check pending" : proxy.upstream.status === "healthy" ? `Upstream ${proxy.upstream.httpStatus} · ${proxy.upstream.responseMs} ms` : `Upstream unavailable · ${escapeHtml(proxy.upstream.error || "check failed")}`;
...
$("#settings-form").addEventListener("submit", async event => { if (state.editing?.kind !== "site") return; event.preventDefault(); event.stopImmediatePropagation(); const button = event.submitter; button.disabled = true; button.textContent = "Applying…"; $("#settings-error").textContent = ""; const form = new FormData(event.target); try { await api("/api/sites/" + state.editing.id, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ domain: form.get("domain"), tls: form.get("tls"), hsts: form.has("hsts"), accessListId: form.get("accessListId") || "" }) }); $("#settings-dialog").close(); await refresh(); toast("Gateway settings applied."); } catch (error) { $("#settings-error").textContent = error.message; } finally { button.disabled = false; button.textContent = "Save & apply"; } }, true);
...
document.querySelector('#settings-form')?.addEventListener('submit', async event => { if (event.eventPhase !== Event.CAPTURING_PHASE || state.editing?.kind !== 'site') return; event.preventDefault(); event.stopImmediatePropagation(); const form = new FormData(event.target); const body = { domain:form.get('domain'), tls:form.get('tls'), hsts:form.has('hsts'), accessListId:form.get('accessListId') || '', compression:form.get('compression') || 'automatic', requestHeaders:parseHeaderLines(form.get('requestHeadersText')), responseHeaders:parseHeaderLines(form.get('responseHeadersText')), hstsSubdomains:form.has('hstsSubdomains'), customConfig:form.get('customConfig') || '' }; try { await api(`/api/sites/${state.editing.id}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)}); $('#settings-dialog').close(); await refresh(); toast('Gateway settings applied.'); } catch (error) { $('#settings-error').textContent = error.message; } }, true);
document.addEventListener('submit', event => { if (event.target.id !== 'settings-form' || state.editing?.kind !== 'site') return; event.preventDefault(); event.stopImmediatePropagation(); const form = new FormData(event.target); const body = { domain:form.get('domain'), tls:form.get('tls'), hsts:form.has('hsts'), accessListId:form.get('accessListId') || '', compression:form.get('compression') || 'automatic', requestHeaders:parseHeaderLines(form.get('requestHeadersText')), responseHeaders:parseHeaderLines(form.get('responseHeadersText')), hstsSubdomains:form.has('hstsSubdomains'), customConfig:form.get('customConfig') || '' }; api(`/api/sites/${state.editing.id}`, {method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(() => { $('#settings-dialog').close(); refresh(); toast('Gateway settings applied.'); }).catch(error => { $('#settings-error').textContent = error.message; }); }, true);
