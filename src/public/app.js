...
$("#settings-form").addEventListener("submit", async event => { if (state.editing?.kind !== "site") return; event.preventDefault(); event.stopImmediatePropagation(); const button = event.submitter; button.disabled = true; button.textContent = "Applying…"; $("#settings-error").textContent = ""; const form = new FormData(event.target); try { await api("/api/sites/" + state.editing.id, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ domain: form.get("domain"), tls: form.get("tls"), hsts: form.has("hsts"), accessListId: form.get("settingsAccessListId") || "" }) }); $("#settings-dialog").close(); await refresh(); toast("Gateway settings applied."); } catch (error) { $("#settings-error").textContent = error.message; } finally { button.disabled = false; button.textContent = "Save & apply"; } }, true);
