...
// Mirrors the same "Active/Disabled + Last run" format the Administration System panel uses
// (see renderSystemStatus in features.js), so the two don't show conflicting detail for the
// same job -- previously this tile only ever showed "Active · <schedule>" with no run info.
function renderDashboardJobsSafe(system) { const slot = document.querySelector("#dashboard-jobs-slot"); if (!slot) return; let panel = document.querySelector("#dashboard-jobs"); if (!panel) { panel = document.createElement("section"); panel.id = "dashboard-jobs"; panel.className = "dashboard-panel dashboard-jobs-panel"; slot.appendChild(panel); } panel.innerHTML = `<div class="panel-heading"><div><p class="eyebrow">Operations</p><h2>Scheduled jobs</h2></div></div><div class="health-grid">${(system.jobs || []).map(job => { const status = job.enabled ? `Active · ${escapeHtml(job.schedule)}` : "Disabled"; const lastRun = job.lastRunAt ? `Last run ${escapeHtml(formatTime(job.lastRunAt))}${job.lastStatus && job.lastStatus !== "ok" ? ` · ${escapeHtml(job.lastStatus)}` : ""}` : "No run recorded yet"; return `<div class="health-tile"><span class="status-dot ${job.enabled ? "running" : "idle"}"></span><span class="health-tile-copy"><strong>${escapeHtml(job.name)}</strong><small>${status} · ${lastRun}</small></span></div>`; }).join("")}</div>`; }
