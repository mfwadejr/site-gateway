...
function renderAccessLists() {
  ...
  list.innerHTML = state.accessLists.length ? state.accessLists.map(item => `<article class="data-row" data-access-id="${item.id}"><span class="status-dot ${item.enabled === false ? "disabled" : "running"}"></span><div><strong>${extendedEscape(item.name)}</strong><small>${item.enabled === false ? "Disabled" : "Available to assign"}</small></div><div><strong>${item.networks?.length || 0}</strong><small>Allowed network${item.networks?.length === 1 ? "" : "s"}</small></div><div><strong>${item.credentials?.length || 0}</strong><small>Login${item.credentials?.length === 1 ? "" : "s"}</small></div><div class="row-actions"><button class="button secondary" data-access-action="edit">Edit</button><button class="button secondary" data-access-action="toggle">${item.enabled === false ? "Enable" : "Disable"}</button><button class="button secondary danger-text" data-access-action="delete">Delete</button></div></article>`).join("") : '<p class="quiet-state padded">No Access Lists yet. Create one to protect a proxy host.</p>';
